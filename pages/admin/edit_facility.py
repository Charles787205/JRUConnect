...
import shutil
class EditFacilityPage(Frame):
  def __init__(self, parent, facility):
    self.parent = parent
    super(EditFacilityPage, self).__init__(parent, bg=WHITE, name='add_facility_page')
  ...
  def add_facility_form(self):
    ...
    self.name_entry.insert(0, self.facility.name)
    self.location_entry.insert(0, self.facility.location)
    self.capacity_entry.insert(0, self.facility.seating_capacity)
    self.image_entry.configure(fg="black", state="normal")
    self.image_entry.delete(0, END)
    self.image_entry.insert(0, self.facility.image_url)
    self.image_entry.configure(textvariable=self.image_entry, state="readonly")
    
    self.edit_facility_button = Button(self.facility_form, text="Edit Facility", font=(MAIN_FONT, 20), bg=WHITE, fg=BLUE, cursor="hand2")
    self.edit_facility_button.grid(row=4, column=1, padx=10, pady=10, sticky="w")
    self.edit_facility_button.bind("<Button-1>", lambda e: self.on_edit_facility_clicked())
  def on_image_entry_click(self, event):
    ...
    if not is_valid_image:
      ...
      self.image_entry.configure(textvariable=self.image_entry, state="normal")
      self.image_entry.configure(fg="black", state="normal")
      self.image_entry.delete(0, END)
  ...
  def on_edit_facility_clicked(self):
    ...
    print(self.facility.image_url, image_url)
    if(self.facility.image_url != image_url):
      source_file = open(image_url, 'rb')
      destination_file = open(f"assets/{image_url.split('/')[-1]}" , 'wb')

      shutil.copyfileobj(source_file, destination_file)
      image_url = image_url.split('/')[-1]
      print(image_url, "asdfs")

    facility = Facility(id=self.facility.id, name=name, location=location, seating_capacity=capacity, image_url=image_url)
    facility.edit()
    messagebox.showinfo("Facility Added", "Facility has been edited successfully")
